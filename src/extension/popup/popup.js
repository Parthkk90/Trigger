...
async function init() {
  ...
  btnBack.addEventListener('click', () => toggleSettings(false));
...
async function toggleSettings(show) {
  if (show) {
    ...
    const viewer = await getViewerUrl();
    const { apiToken } = await _browser.storage.sync.get('apiToken');

    inputBackendUrl.value = backend || '';
    inputViewerUrl.value = viewer || '';
    inputApiToken.value = apiToken || '';

    workflowsPanel.style.display = 'none';
    controls.style.display = 'none';
    statusBar.style.display = 'none';
    settingsPanel.style.display = 'block';
  } else {
    settingsPanel.style.display = 'none';
    workflowsPanel.style.display = 'block';
...
async function saveSettings() {
  const backendInput = inputBackendUrl.value.trim();
  const viewerInput = inputViewerUrl.value.trim();
  const tokenInput = inputApiToken.value.trim();

  const backendSanitized = backendInput ? sanitizeUrl(backendInput) : null;
  const viewerSanitized = viewerInput ? sanitizeUrl(viewerInput) : null;

  if (backendInput && !backendSanitized) {
    showNotice('Invalid backend URL. Use http://host:port or https://host', 'error');
    return;
  }
  if (viewerInput && !viewerSanitized) {
  ...
  if (backendSanitized) {
    ...
    await _browser.storage.sync.remove('backendUrl');
  }

  if (viewerSanitized) {
    await _browser.storage.sync.set({ viewerUrl: viewerSanitized });
  } else {
    await _browser.storage.sync.remove('viewerUrl');
  }

  if (tokenInput) {
    await _browser.storage.sync.set({ apiToken: tokenInput });
  } else {
    await _browser.storage.sync.remove('apiToken');
  }
  ...
  workflowList.innerHTML = workflows.map(w => `
    <div class="workflow-item" data-id="${w.id}">
      <div class="workflow-info">
        ...
        <div class="workflow-meta">${w.steps.length} steps · ${formatLastRunStatus(w)}</div>
      </div>
      <div class="workflow-actions">
        <button class="btn-play" data-action="play" data-id="${w.id}" title="Play workflow" aria-label="Play workflow ${escapeHtml(w.name)}">▶</button>
        <button class="btn-share" data-action="share" data-id="${w.id}" title="Share workflow" aria-label="Share workflow ${escapeHtml(w.name)}">🔗</button>
        <button class="btn-delete" data-action="delete" data-id="${w.id}" title="Delete workflow" aria-label="Delete workflow ${escapeHtml(w.name)}">✕</button>
      </div>
    </div>
  `).join('');
}
...
function sendMessage(msg) {
  return _browser.runtime.sendMessage(msg);
}

async function getViewerUrl() {
  const result = await _browser.storage.sync.get('viewerUrl');
...
function formatLastRunStatus(workflow) {
  if (workflow && workflow.lastRunStatus) {
    return `last run: ${workflow.lastRunStatus}`;
  }
  if (workflow && workflow.lastReplayedAt) {
    return `last run: ${formatDate(workflow.lastReplayedAt)}`;
  }
