...
(function () {
    ...
    if (step.type === 'navigate') {
      if (step.spa) {
        return waitForUrl(step.url, 5000).then(function () {
          return evaluateAuthWallAfterSettle(step);
        });
      }
      return evaluateAuthWallAfterSettle(step);
    }

    var captchaInfo = detectCaptchaChallenge();
    if (captchaInfo.detected) {
      return Promise.resolve({
        success: false,
        confidence: 0,
        reason: 'CAPTCHA challenge detected (' + captchaInfo.provider + '). Solve manually and continue.',
        reasonType: 'captcha_challenge',
      });
    }
  ...
  window.Trigger.evaluateReplayFreshness = function (sampleSteps) {
    var steps = Array.isArray(sampleSteps) ? sampleSteps : [];
    var scored = [];

    for (var i = 0; i < steps.length; i += 1) {
      var step = steps[i];
      if (!step || !step.target) continue;
      var result = window.Trigger.resolveFingerprint(step.target) || {};
      scored.push({
        stepIndex: typeof step.index === 'number' ? step.index : i,
    ...
    var total = scored.reduce(function (acc, item) { return acc + item.confidence; }, 0);
    return {
      averageConfidence: total / scored.length,
      sampledCount: scored.length,
      details: scored,
    };
  };
  ...
  function classifyReplayErrorType(message) {
    ...
    if (text.indexOf('auth wall') !== -1 || text.indexOf('sign in required') !== -1 || text.indexOf('permission error') !== -1) return 'auth_wall';
    if (text.indexOf('captcha') !== -1 || text.indexOf('recaptcha') !== -1 || text.indexOf('hcaptcha') !== -1 || text.indexOf('turnstile') !== -1) return 'captcha_challenge';
  ...
  function detectAuthWallNavigation(step) {
    var expectedUrl = step && step.url ? step.url : '';
    var currentUrl = window.location.href || '';

    if (!isSignificantUrlMismatch(expectedUrl, currentUrl)) {
      return { detected: false };
    }

    if (hasAuthUrlSignal(currentUrl) || hasAuthPageSignal()) {
  ...
  function evaluateAuthWallAfterSettle(step) {
    return sleep(NAV_AUTH_SETTLE_MS).then(function () {
      var authWallInfo = detectAuthWallNavigation(step);
      if (authWallInfo.detected) {
        return {
          success: false,
          confidence: 0,
          reason: 'Permission error: auth wall detected. Sign in required before replay can continue.',
          reasonType: 'auth_wall',
        };
      }
      return { success: true };
    });
  ...
  function isSignificantUrlMismatch(expectedUrl, currentUrl) {
    try {
      var expected = new URL(expectedUrl, window.location.origin);
      var current = new URL(currentUrl, window.location.origin);
      if (expected.origin !== current.origin) return true;
      var expectedPath = normalizePath(expected.pathname);
      var currentPath = normalizePath(current.pathname);
      if (expectedPath === currentPath) return false;
      if (currentPath.indexOf(expectedPath) === 0 || expectedPath.indexOf(currentPath) === 0) return false;
      return true;
    } catch (err) {
  ...
  function hasAuthUrlSignal(url) {
    var text = String(url || '').toLowerCase();
    return /signin|sign-in|login|auth|sso/.test(text);
  }

  function hasAuthPageSignal() {
  ...
  function detectCaptchaChallenge() {
    ...
    for (var i = 0; i < selectors.length; i++) {
      ...
      var provider = 'captcha';
      if (src.indexOf('recaptcha') !== -1) provider = 'recaptcha';
      else if (src.indexOf('hcaptcha') !== -1) provider = 'hcaptcha';
      else if (src.indexOf('challenges.cloudflare.com') !== -1) provider = 'cloudflare_turnstile';
